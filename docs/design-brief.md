...
**Historical.** This is the original pre-implementation design brief, kept for
the rationale (why one system spans every guest type, why the contract is
load-bearing, the overhead evidence). It predates the code and does not track it:
the working name `spine` became Assayist, and the coverage matrix below lists
spaces and adapters as aspirations, not as built. What actually shipped is
firecracker, qemu, cloud-hypervisor, and unikernel targets with fio, wrk, and
vsock workloads; see [`architecture.md`](architecture.md) for the current system
and the [root README](../README.md) status table for what is built and run. The
open questions at the foot of this doc (name, schema backbone, licence) are all
decided.
