...
/// Pick a target adapter by name. `firecracker`, `qemu`, `cloud-hypervisor`
/// (alias `chv`), and `unikernel` are native adapters; anything else falls back
/// to the command adapter (shell templates from the def). `pin_threads` comes
/// from `host_prep`; only the firecracker target acts on it today.
fn select_target(def: &BenchmarkDef, vars: BTreeMap<String, String>, pin_threads: bool) -> Box<dyn Target> {
    match def.target.adapter.as_str() {
        ...
        "unikernel" => Box::new(native::unikernel_target(&def.target.config, &vars, pin_threads)),
