...
- **log2-derived percentiles are coarse.** `p50`/`p99` off a 40-slot log2 histogram snap to bucket midpoints (`1.5 * 2^n`), so their run-to-run CoV is high and the noise gate usually excludes them. That is the correct behaviour (do not gate on quantised tails), so by default **gate on the mean** (from the exact `sum` the gadgets emit) and treat log2 percentiles as advisory. When you do need to gate a tail, capture a finer histogram for that metric: the reduce path already handles `layout: explicit` (arbitrary `explicit_bounds`), and the block gadget's `--hires` emits a log-linear histogram (each octave split into 4 sub-buckets, ~4x finer p99), whose percentiles fall within the noise gate rather than snapping across a whole octave.
